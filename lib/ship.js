(function() {
  ...
  Ship.prototype.power = function (impulse, dir) {
    ...
    if (Math.abs(Asteroids.Util.norm(this.vel)) >= Ship.MAX_VEL) {
      this.resetSpeedToMax();
    }
  };

  Ship.prototype.resetSpeedToMax = function () {
    var velDir = Asteroids.Util.vectorDir(this.vel);
    this.vel = Asteroids.Util.vectorByDirAndNorm(velDir, Ship.MAX_VEL);
  };
